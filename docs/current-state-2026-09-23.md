...
Florida Man and HOA President now have accepted-for-test 25-Health packages with passives, regular abilities, Charge, and ultimates. The [duel packet](../production/playtests/florida-vs-hoa/README.md) contains legal two-Style 40-card decks, full card references, leader timing rulings, and an exploratory test report. HOA's original opponent-dependent Block Charge starved in testing, so **Document Everything** now gains its once-per-Round Charge from the first friendly Character or Item ability the HOA player Activates. This made Charge accessible, while the limited computer pilots still showed policy-sensitive stalls and no HOA ultimate use; no competitive win-rate or balance claim is established. The remaining four Leader packages are still open.
