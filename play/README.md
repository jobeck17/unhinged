...
**Current live build: [Character Lab 1](../production/playtests/character-lab/README.md).** The app now loads that folder’s cards and decks plus `engine-lab.js` / `ai-lab.js`. Run `node play/test-character-lab.mjs` and `node play/test-app.mjs`. Details below describe the prior six-deck baseline.
