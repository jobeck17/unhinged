...
**Current live build: [Character Lab 2](../production/playtests/character-lab/README.md).** Leaders are now deck identity and 25 Health only; the app and lab engine contain no Leader actions, Charge, rotation, passives, or ultimates. The app loads that folder’s cards and decks plus `engine-lab.js` / `ai-lab.js`. Run `node play/test-character-lab.mjs` and `node play/test-app.mjs`. Details below describe the prior six-deck baseline.
