...
export function aiAction(g,p){let s=g.players[p],own=g.chars(p),opp=g.chars(1-p),name=g.name(p);
 ...
 if(name==='Florida Man'&&s.ready&&!s.passive&&attackers.some(x=>g.guard(x)-x.damage>1)&&s.charge<3)return {type:'leader'};
 ...
 let ultimate=s.ready&&s.charge===3&&(name!=='Backyard Wrestler'||own.length)&&(name!=='HOA President'||opp.some(x=>!x.ready));if(ultimate)return {type:'ultimate'};
 ...
 if(name==='Washed-Up Rock Star'&&s.ready&&s.hand.length<5&&s.deck.length)return {type:'leader'};
