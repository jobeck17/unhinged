...
export class Game{
 ...
 async damage(x,n,defer=false){if(!x||!this.obj(x.uid)||n<=0||x.cloaked)return;let shield=this.items(x).find(i=>i.id==='P149');if(shield){let use=await this.choose(x.owner,'Dismiss Occupied Stroller to prevent 2 damage?',[{label:'Dismiss and prevent 2',value:true},{label:'Keep stroller',value:false}]);if(use){await this.dismiss(shield);n=Math.max(0,n-2)}}if(!n)return;x.damage+=n;this.say(`${this.card(x).name} takes ${n} damage`);if(!defer)await this.checkDefeat(x)}
 async combatDamage(pairs){for(let [x,n] of pairs)await this.damage(x,n,true);let dead=pairs.map(([x])=>x).filter(x=>this.obj(x.uid)&&!x.cloaked&&x.damage>=this.guard(x));for(let x of dead){x.cargoRevealed=[...x.cargo];this.say(`${this.card(x).name} is Defeated`);await this.remove(x,'discard',false)}for(let x of dead){let p=x.owner;this.players[p].defeatedRound=true;for(let id of this.layers(x))await this.defeatEffect(id,x,p,x.lower)}}
 ...
 async dismiss(x){if(x&&this.obj(x.uid))await this.remove(x)}
 ...
 if(s.played.length===2)for(let item of s.board.filter(z=>z.id==='P057')){let z=this.obj(item.attached);if(z)z.power++}
 return true}
 ...
 case 'P079':{let i=await this.pickHand(p,'Present a face-down card');if(i!=null){let card=this.card(s.hand[i]);let guess=await this.choose(opp,'Pick a Card: guess its type',[{label:'Character',value:true},{label:'Not Character',value:false}]);this.say(`Reveals ${card.name}`);if(card.id==='P062'&&s.hand.includes('P062')){let yes=await this.choose(p,'Birthday Kid: put revealed card on bottom and Draw?',[{label:'Yes',value:true},{label:'No',value:false}]);if(yes){let pos=s.hand.indexOf('P062');s.deck.unshift(...s.hand.splice(pos,1));this.draw(p)}}if(guess===(card.type==='Character'))await this.rummage(p);else this.draw(p,2)}break}
 ...
 if(target===-1){let blocker=blockers[0];if(blocker&&this.obj(blocker.uid)){let absorbed=Math.min(incomingDamage,Math.max(0,this.guard(blocker)-blocker.damage)),retal=this.power(blocker);this.hurtLeader(opp,incomingDamage-absorbed);await this.combatDamage([[blocker,absorbed],[a,retal]])}else this.hurtLeader(opp,incomingDamage)}else{let x=this.obj(target);await this.combatDamage([[x,incomingDamage],[a,this.power(x)]])}
 this.advance()}
}
