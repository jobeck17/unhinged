...
export class Game{
 ...
 async pickHand(p,title,condition=()=>true,optional=false){let s=this.players[p],o=s.hand.map((id,i)=>({label:`${this.card(id).name} · Cost ${this.card(id).cost}`,value:i})).filter(o=>condition(this.card(s.hand[o.value]),o.value));return this.choose(p,title,o,optional)}
 ...
 startTurn(){for(let s of this.players){Object.assign(s,{played:[],leaderPassiveUsed:false,attacked:false,prevent:0,defeatedRound:false,nextCharDiscount:0,nextItemDiscount:0,friendPower:false,discountUndead:null});for(let x of s.board){x.once=false;x.roundUsed=false;x.noBlock=false}}let p=this.turn,s=this.players[p];s.stashedThisTurn=false;s.fuel=s.stash.length;for(let x of s.board){x.ready=true;x.attacked=false}this.draw(p,1,false);this.checkEnd();this.update?.()}
 async endRound(){let nextRound=this.turn!==this.first;for(let p=0;p<2;p++)for(let x of [...this.players[p].board]){if(!this.obj(x.uid))continue;if(x.id==='P029'){this.hurtLeader(x.beside,1);this.say(`Hot Potato hits ${this.name(x.beside)}`);await this.remove(x,'hand')}if(x.id==='P089'&&x.hidden){this.players[p].hand.push(x.hidden);x.hidden=null}if(x.cloaked&&nextRound)x.cloaked=false;if(this.card(x).type==='Character'){x.power=0;x.guard=0;x.hot=false;await this.checkDefeat(x)}}this.checkEnd();if(this.winner!==null)return;this.turn=1-this.turn;if(nextRound){this.round++;this.startRound()}else this.startTurn()}
 async pass(){this.say('End turn');await this.endRound()}
 advance(){this.pendingAttack=null;this.checkEnd();this.update?.()}
 enter(p,id,attach=null,lower=null,cardOwner=p){let x={uid:++this.nextUid,id,lower,owner:p,cardOwner,ready:true,born:this.round,damage:0,power:0,guard:0,hot:false,attacked:false,attached:attach,cargo:[],cloaked:false};this.players[p].board.push(x);return x}
 async remove(x,where='discard',defeated=false){if(!x||!this.obj(x.uid))return;let p=x.owner,s=this.players[p],owner=x.cardOwner??p,dest=this.players[owner],wasCharacter=this.card(x).type==='Character',rabbit=this.has(x,'P063');if(this.items(x).some(i=>i.uid===x.uid))throw Error(`Self attached ${x.id} ${x.uid}`);for(let i of [...this.items(x)])await this.remove(i);if(x.id==='P089'&&x.hidden){s.hand.push(x.hidden);x.hidden=null}if(x.cargo?.length){s.discard.push(...x.cargo);x.cargo=[]}s.board=s.board.filter(y=>y.uid!==x.uid);let lower=x.lower;dest[where].push(...(lower?[lower]:[]),x.id);if(defeated){s.defeatedRound=true;await this.leaderPassive(p,'friendlyDefeat');for(let id of lower?[lower,x.id]:[x.id])await this.defeatEffect(id,x,p,lower)}else if(where==='hand'){if(wasCharacter&&owner===p)await this.leaderPassive(p,'returnedCharacter');if(rabbit&&owner===p){this.draw(p);this.say('Rabbit returns to the hat and draws a card')}for(let y of this.chars(p))if(y.uid!==x.uid&&!y.cloaked&&this.has(y,'P066')&&!y.once){y.once=true;await this.rummage(p)}}}
 ...
 canStash(index,p=this.turn){let s=this.players[p];return !s.stashedThisTurn&&index>=0&&index<s.hand.length}
 stash(index,p=this.turn){let s=this.players[p];if(!this.canStash(index,p))return false;let id=s.hand.splice(index,1)[0];s.stash.push(id);s.stashedThisTurn=true;s.fuel++;this.say(`${this.name(p)} Stashes a card · ${s.stash.length} total`);this.update?.();return true}
 ...
 s.fuel-=cost;let previous=[...s.played];s.played.push({id,type:c.type});this.say(`Plays ${c.name}${cost?` (Cost ${cost})`:' for free'}`);
 if(c.type==='Character'){
 ...
 if(id==='P061'){let t=await this.pick("Magician's Assistant: Return another friendly Character",p,this.chars(p).filter(y=>y.uid!==x.uid),true);if(t!=null)await this.remove(this.obj(t),'hand')}
 if(id==='P064'&&this.players[opp].deck.length){let revealed=this.players[opp].deck.pop(),card=this.card(revealed);this.say(`Volunteer reveals ${card.name}`);if(card.type==='Character'){let use=await this.choose(p,`Dismiss Volunteer From the Audience to put ${card.name} into play?`,[{label:'Dismiss Volunteer and put it in play',value:true},{label:'Put it on the bottom',value:false}]);if(use){await this.dismiss(x);this.enter(p,revealed,null,null,opp)}else this.players[opp].deck.unshift(revealed)}else this.players[opp].deck.unshift(revealed)}
 ...
 canUse(x){if(['P032','P102','P165'].includes(x?.id))return false;if(!this.canActivate(x)||x.owner!==this.turn&&!(x.id==='P029'&&x.beside===this.turn))return false;if(x.id==='P119'&&!this.players[this.turn].board.some(y=>y.uid!==x.uid&&this.card(y).type==='Item'&&!y.ready))return false;if(['P032','P059','P074','P102','P124','P126','P165','P166'].includes(x.id)&&!this.chars(this.turn).length)return false;if(['P003','P126','P165','P166'].includes(x.id)&&!this.chars(this.turn).some(y=>y.uid!==x.uid))return false;if(['P138','P147'].includes(x.id)&&!this.chars(1-this.turn).length)return false;if(x.id==='P120'&&![...this.chars(this.turn),...this.chars(1-this.turn)].some(y=>y.damage)||x.id==='P124'&&!this.chars(this.turn).some(y=>y.damage))return false;if(x.id==='P166'&&!this.chars(this.turn).some(y=>y.uid!==x.uid&&y.damage))return false;if(x.id==='P088'&&!this.players[this.turn].deck.length)return false;if(x.id==='P166'&&this.chars(this.turn).length<2)return false;return this.layers(x).some(id=>!['P032','P102','P165'].includes(id)&&['P003','P027','P032','P059','P074','P088','P102','P117','P119','P120','P124','P126','P132','P138','P147','P165','P166','P178','P179'].includes(id))||x.id==='P029'&&x.beside===this.turn||x.id==='P063'||x.id==='P066'}
 async activate(uid,mode=null){let p=this.turn,x=this.obj(uid);if(!x||!this.canUse(x)||x.owner!==p&&!(x.id==='P029'&&x.beside===p))return;let opts=this.layers(x).filter(id=>!['P032','P102','P165'].includes(id)&&['P003','P027','P032','P059','P074','P088','P102','P117','P119','P120','P124','P126','P132','P138','P147','P165','P166','P178','P179'].includes(id));if(x.id==='P029')opts.push('P029');if(!opts.length)return;let id=mode||(opts.length===1?opts[0]:await this.choose(p,'Choose an ability',opts.map(z=>({label:z==='Cloak'?'Cloak':this.card(z).name,value:z}))));if(!id)return;let own=this.chars(p),opp=this.chars(1-p),target=null,second=null;
 let lists={P003:own.filter(y=>y.uid!==x.uid),P032:own,P059:own,P074:own.filter(y=>y.uid!==x.uid),P102:own.filter(y=>y.uid!==x.uid),P120:[...own,...opp].filter(y=>y.damage),P124:own.filter(y=>y.damage),P126:own.filter(y=>y.uid!==x.uid),P132:[...own.filter(y=>y.damage),...(this.players[p].hp<25?[-1]:[])],P138:opp,P147:opp,P165:own.filter(y=>y.uid!==x.uid),P166:own.filter(y=>y.uid!==x.uid&&y.damage)};
