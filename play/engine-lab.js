...
export class Game{
 ...
 case 'P079':{if(s.hand.length<3)this.draw(p,3-s.hand.length);if(this.winner!==null||!s.hand.length)break;let i=await this.choose(opp,'Pick a Card: choose one face-down card',s.hand.map((_,i)=>({label:`Face-down card ${i+1}`,value:i})));if(i!=null&&s.hand[i]){let chosen=s.hand[i],card=this.card(chosen);this.say(`Pick a Card reveals ${card.name}`);await this.playCard(p,chosen,'hand',0,{index:i})}break}
