...
export const LEADERS={
 'Florida Man':{style:'Reckless',passive:'Walk It Off: First friendly survivor of damage during your Turn gets +1 Power this turn.',ability:'Watch This: Rotate; deal 1 to a Ready friendly Character. If it survives, +2 Power this turn.',charge:'Gain Charge when Walk It Off triggers.',ultimate:'Hold My Entire Cooler: Roll d6 damage to an opposing Character or Leader.'},
 'Washed-Up Rock Star':{style:'Momentum',passive:'Crowd Warms Up: The second card you Play each turn gives a friendly Character +1 Power this turn.',ability:'Sound Check: Rotate; Draw, then Discard.',charge:'Gain Charge on the second card you Play each turn.',ultimate:'The Reunion Tour: Play up to two Characters from hand with combined printed Cost at most 4 for free; one gains Hothead this turn.'},
 'Birthday Party Magician':{style:'Misdirection',passive:'Sleight of Hand: First friendly Character Returned from play to hand each turn lets you Draw, then Discard.',ability:'Check Behind Your Ear: Rotate; look at the top two cards, put one on bottom and one back on top.',charge:'Gain Charge the first time your effect reveals a card from hand or Returns a friendly Character from play each turn.',ultimate:'The Disappearing Act: Choose up to two opposing Characters; their owner chooses one to Return and the other Rotates. Play a Character costing at most 3 from hand for free.'},
 'Trash Baron':{style:'Salvage',passive:'Waste Not: First Item you Dismiss each turn deals 1 to the opposing Leader.',ability:"One Man's Trash: Rotate and Dismiss your Item; Return a different Item costing at most 2 from discard to hand.",charge:'Gain Charge on your first Item Play from hand or Item Dismiss each turn.',ultimate:'The Junkyard Kingdom: Play up to two Items costing at most 2 from discard for free.'},
 'HOA President':{style:'Stonewall',passive:'Neighborhood Watch: First friendly Blocker each turn gets +1 Power for that Attack.',ability:'Property Maintenance: Rotate; heal 2 from your Character.',charge:'Gain Charge on the first friendly Character or Item activation each turn.',ultimate:'Violation Notice: Deal 2 to up to two opposing Rotated Characters.'},
 'Backyard Wrestler':{style:'Expendable',passive:'Crowd Goes Wild: First friendly Defeat each turn deals 1 to opposing Leader, including Sacrifice.',ability:'Set Up the Table: Rotate, Sacrifice your Character; deal 2 to opposing Character.',charge:'Gain Charge on first friendly Defeat each turn, including Sacrifice.',ultimate:'Through the Table: Sacrifice your Character; deal its Power to an opposing Character or Leader, plus 2 to opposing Leader if you chose a Character.'}
};
...
export class Game{
 ...
 async damage(x,n,defer=false){if(!x||!this.obj(x.uid)||n<=0||x.cloaked)return;let shield=this.items(x).find(i=>i.id==='P149');if(shield){let use=await this.choose(x.owner,'Dismiss Occupied Stroller to prevent 2 damage?',[{label:'Dismiss and prevent 2',value:true},{label:'Keep stroller',value:false}]);if(use){await this.dismiss(shield);n=Math.max(0,n-2)}}if(!n)return;x.damage+=n;this.say(`${this.card(x).name} takes ${n} damage`);if(!defer&&await this.checkDefeat(x))return;if(defer&&x.damage>=this.guard(x))return;let p=x.owner,s=this.players[p];if(this.name(p)==='Florida Man'&&this.turn===p&&!s.passive){s.passive=true;x.power++;this.gain(p);for(let y of this.chars(p))if(y.uid!==x.uid&&!y.cloaked&&this.has(y,'P066')&&!y.once){y.once=true;await this.rummage(p)}}}
 async combatDamage(pairs){for(let [x,n] of pairs)await this.damage(x,n,true);let dead=pairs.map(([x])=>x).filter(x=>this.obj(x.uid)&&!x.cloaked&&x.damage>=this.guard(x));for(let x of dead){x.cargoRevealed=[...x.cargo];this.say(`${this.card(x).name} is Defeated`);await this.remove(x,'discard',false)}for(let x of dead){let p=x.owner,s=this.players[p];s.defeatedRound=true;if(this.name(p)==='Backyard Wrestler'&&!s.passive){s.passive=true;this.hurtLeader(1-p,1);this.gain(p)}for(let id of this.layers(x))await this.defeatEffect(id,x,p,x.lower)}}
 ...
 if(target===-1){let blocker=blockers[0];if(blocker&&this.obj(blocker.uid)){let absorbed=Math.min(incomingDamage,Math.max(0,this.guard(blocker)-blocker.damage)),retal=this.power(blocker)+(hoa===blocker.uid?1:0);this.hurtLeader(opp,incomingDamage-absorbed);await this.combatDamage([[blocker,absorbed],[a,retal]])}else this.hurtLeader(opp,incomingDamage)}else{let x=this.obj(target);await this.combatDamage([[x,incomingDamage],[a,this.power(x)]])}
 this.advance()}
}
