...
export class Game{
 ...
 say(msg){this.log.unshift(`R${this.round} · ${this.name(this.turn)}: ${msg}`);this.log.length=Math.min(100,this.log.length);this.update?.()}
 ...
 startRound(){for(let p=0;p<2;p++){let s=this.players[p];Object.assign(s,{ready:true,fuel:Math.min(7,this.round),gained:false,passive:false,played:[],sneakyUsed:false,attacked:false,prevent:0,defeatedRound:false,nextCharDiscount:0,nextItemDiscount:0,friendPower:false,discountUndead:null});for(let x of s.board){x.ready=true;x.attacked=false;x.roundUsed=false;x.power=0;x.guard=0;x.hot=false;x.once=false}this.draw(p,1,false)}this.say(`Round ${this.round} begins · ${Math.min(7,this.round)} Fuel`);this.checkEnd()}
 async endRound(){for(let p=0;p<2;p++)for(let x of [...this.players[p].board]){if(x.id==='P029'){this.hurtLeader(x.beside,1);this.say(`Hot Potato hits ${this.name(x.beside)}`);await this.remove(x,'hand')}if(x.id==='P089'&&x.hidden){this.players[p].hand.push(x.hidden);x.hidden=null}if(x.cloaked){x.cloaked=false;await this.checkDefeat(x)}if(this.card(x).type==='Character'){x.power=0;x.guard=0;x.hot=false;await this.checkDefeat(x)}}this.checkEnd();if(this.winner!==null)return;this.round++;this.turn=this.first;this.startRound()}
 async pass(){this.say('Pass');if(this.passed!==null){this.first=this.passed;this.passed=null;await this.endRound()}else{this.passed=this.turn;this.turn=1-this.turn;this.players[this.turn].sneakyUsed=false;this.update?.()}}
 advance(free=false){this.passed=null;this.pendingAttack=null;this.checkEnd();if(!free&&this.winner===null){this.turn=1-this.turn;this.players[this.turn].sneakyUsed=false}this.update?.()}
 ...
 this.advance()}
 async attack(uid){let p=this.turn,a=this.obj(uid),opp=1-p;if(!a||a.owner!==p||!this.canAttack(a))return;let possible=[-1,...this.chars(opp).filter(x=>!x.ready||this.layers(a).some(id=>this.card(id).keywords.includes('Sucker Punch')))],target=await this.pick('Attack which target?',p,possible);if(target==null)return;
