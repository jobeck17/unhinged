...
await import('./app.js');assert.equal(buttons.length,6,'six Leaders shown');assert(html.includes('Backyard Wrestler'));assert(html.includes('Character Lab 1'));assert(html.includes('Vacuum Goes BRRR'));assert(html.includes('third Action'));assert.equal((html.match(/★ /g)||[]).length,3,'three featured decks');
