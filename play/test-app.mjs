...
await import('./app.js');assert.equal(buttons.length,6,'six Leaders shown');assert(html.includes('Backyard Wrestler'));assert(html.includes('Character Lab 2'));assert(html.includes('sets your deck identity'));assert(html.includes('passive-preview'));assert(!html.includes('Charge')&&!html.includes('Ultimate')&&!html.includes('Ability used')&&!html.includes('data-leader'));assert(html.includes('Vacuum Goes BRRR'));assert(html.includes('third Action'));assert(html.includes('Pirate With a Business License'));assert.equal((html.match(/★ /g)||[]).length,4,'four featured decks');
