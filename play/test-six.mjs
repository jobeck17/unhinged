...
for(let a=0;a<6;a++)for(let b=0;b<6;b++){if(a===b)continue;let decks={...full,decks:[full.decks[a],full.decks[b]]};let g=new Game(pool,decks,r=>aiChoice(g,r),()=>{});g.begin();let limit=1250;
 while(g.winner===null&&g.round<=35&&limit--){let move=aiAction(g,g.turn),turn=g.turn,round=g.round,playedId=g.players[g.turn].hand[move.index];
  ...
  if(!(g.winner!==null||g.turn!==turn||g.round!==round||move.type==='play'&&g.players[turn].sneakyUsed))throw Error(`action stuck: ${g.name(turn)} ${JSON.stringify(move)} R${round}; card ${g.obj(move.uid)?.id||playedId}; ${g.log.slice(0,6).join(' | ')}`);
