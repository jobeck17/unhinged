# Salvage / Good Enough — Production Pool v0.1

> Donut revision 4 · 2026-09-24 · Working playtest text; balance is unverified.
> Generated from [cards.json](cards.json). Edit the source and run `python3 production/cards/build.py`.

| ID | Type | Cost | Card | Power / Guard | Traits | Working text |
| --- | --- | --- | --- | --- | --- | --- |
| P091 | Character | 2 | **Scout Troop Quartermaster** | 3/3 | Scout, Scavenger | — |
| P092 | Character | 5 | **General Contractor** | 4/6 | Builder | Rotate: Ready one of your Rotated Items. Activate only once each Round. |
| P093 | Character | 4 | **Garage Inventor** | 3/5 | Builder | Rotate, Dismiss one of your Items: Draw 2 cards, then Discard a card. |
| P094 | Character | 2 | **Dumpster Diver** | 2/3 | Scavenger | When this enters play, look at the top 3 cards of your deck. You may reveal an Item and put it into your hand. Put the rest on the bottom in any order. |
| P095 | Character | 4 | **Doomsday Prepper** | 4/6 | Scavenger | — |
| P096 | Character | 4 | **Shopping Cart Knight** | 5/6 | Scavenger | **Slowpoke**. |
| P097 | Character | 2 | **Junkyard Dog** | 3/3 | Animal, Scavenger | — |
| P098 | Character | 5 | **Raccoon of Unusual Size** | 6/5 | Animal, Scavenger | When this enters play, you may put an Item from your discard on the bottom of your deck. If you do, Draw a card. When this attacks, you may Dismiss one of your Items. If you do, this gets +2 Power for this Attack. |
| P099 | Character | 3 | **Crazy Cat Lady** | 2/5 | — | Rotate: Look at the top 3 cards of your deck. You may reveal a Character and put it into your hand. Put the rest on the bottom in any order. If you revealed an Animal, heal 1 damage from one of your Characters. |
| P100 | Character | 3 | **Wind-Up Toy** | 2/4 | Construct | **Sucker Punch**. When this attacks a Ready Character, this gets +1 Power for that Attack. |
| P101 | Character | 3 | **Extension Cord Electrician** | 3/4 | Builder | When this enters play, you may Ready one of your Items. If you have another Builder in play, Draw a card, then Discard a card. |
| P102 | Character | 5 | **Rat King** | 4/6 | Animal, Rat | Rotate: Choose up to two of your other Characters. Each gets +1 Power this Round. Each chosen Rat also gets +1 Guard this Round. |
| P103 | Character | 2 | **Possum in the Trash Can** | 2/3 | Animal, Scavenger | When this is Defeated, you may Dismiss one of your Items. If you do, Return this from your discard to your hand. |
| P104 | Character | 1 | **Trash Palace Cockroach** | 1/2 | Animal, Scavenger | When this is Defeated, you may put it from your discard on the bottom of your deck. |
| P105 | Character | 3 | **Jailbroken Robot Vacuum** | 2/4 | Construct, Scavenger | When this attacks, put the top card of your deck face down beneath this if able. This gets +1 Power for this Attack per card beneath it. When this is Defeated, reveal and put them into your discard. You may deal damage to an opposing Character equal to the number of Items revealed. If this leaves play another way, reveal and put them into your discard. |
| P106 | Character | 2 | **Shopping Cart Golem** | 2/4 | Construct, Scavenger | **Stack**. When this is Defeated while stacked, Return its lower card from your discard to its Owner's hand. |
| P107 | Character | 3 | **Curbside Mechanic** | 3/4 | Builder, Scavenger | When this enters play, you may Return an Item costing 1 from your discard to your hand. |
| P108 | Character | 6 | **Unlicensed Pool Guy** | 6/7 | Builder | The first Item you Play each Round costs 1 less. Rotate: Return an Item from your discard to your hand. |
| P109 | Action | 1 | **Scrounge** | — | — | Look at the top 3 cards of your deck. You may reveal an Item and put it into your hand. Put the rest into your discard. |
| P110 | Action | 1 | **I Can Fix That** | — | — | Return an Item costing 1 from your discard to your hand. |
| P111 | Action | 2 | **Found This in the Garage** | — | — | Reveal the top 4 cards of your deck. You may put an Item among them into your hand. Put the rest into your discard. |
| P112 | Action | 1 | **That's Probably Still Good** | — | — | Heal 2 damage from a Character, or 3 damage if it is a Scavenger. |
| P113 | Action | 2 | **Make It Work** | — | — | Dismiss one of your Items. If you do, Draw 2 cards. The next Item you Play this Round costs 1 less. |
| P114 | Action | 1 | **Curb Alert** | — | — | **Sneaky**. Put an Item from your discard on the bottom of your deck. If you do, Draw a card. |
| P115 | Action | 2 | **Don't Throw That Away** | — | — | Return an Item that entered your discard this Round to your hand. |
| P116 | Action | 2 | **Good as New** | — | — | Choose a damaged Character. Heal all damage from it, then Rotate it. |
| P117 | Item | 2 | **Duct Tape** | — | — | Attach to one of your Characters. It gets +1 Guard. Rotate: Heal 1 damage from the attached Character, or 2 damage if it is a Construct. |
| P118 | Item | 1 | **Zip Ties** | — | — | Attach to one of your Characters. Dismiss this: The attached Character gets +2 Guard this Round, or +3 Guard if it is a Builder. |
| P119 | Item | 2 | **Mystery Drawer of Cables** | — | — | When this enters play, Draw a card, then Discard a card. Rotate, Dismiss this: Ready another of your Items. |
| P120 | Item | 1 | **Used Ham Sandwich** | — | — | Dismiss this: Heal 2 damage from a Character. If it is a Scavenger, Draw a card, then Discard a card. |

A dash in Working text means no rules text. Traits are still active labels. See [Traits](traits.md), [Keywords](keywords.md), and [Rules](../rules/unhinged-rules.md).

## Flavor text

These optional lines are not rules text and do not change a card’s complexity category.

| Card | Flavor |
| --- | --- |
| P091 Scout Troop Quartermaster | *Prepared for everything except the inspection.* |
| P095 Doomsday Prepper | *End times. Curbside pickup.* |
| P096 Shopping Cart Knight | *Sir Carts-a-Lot yields to no curb.* |
| P097 Junkyard Dog | *Beware of dog. Also tetanus.* |
| P099 Crazy Cat Lady | *Every cat has a name. The neighbors have numbers.* |
| P100 Wind-Up Toy | *No batteries required. No brakes included.* |
| P105 Jailbroken Robot Vacuum | *The knife attachment voided the warranty.* |
| P106 Shopping Cart Golem | *One wheel squeaks. One wheel seeks revenge.* |
