...
- **Birthday Party Magician / Pick a Card, Any Card:** Magician's Assistant Returns another friendly Character on entry. Rabbit draws when one of your effects Returns it, pairing with Sleight of Hand for repeatable hand cycling. Volunteer From the Audience reveals the opponent's top card and may Dismiss itself to put a revealed Character into play under your control; declined or non-Character reveals go to the bottom. Escape Artist is a 5-cost 7/1 with Cloak.
...
6. No Guard Discards. Defiant, Slowpoke, and Sneaky are retired in this variant: their old text is removed from its pool. Cloak is triggered only by retaliation damage: the first time each Round a Character with Cloak would take retaliation damage, turn it face down until the end of the Round instead. Stack, Hothead, Explosive, and Sucker Punch remain subject to the variant timing above.
