...
These are the latest unresolved items from the September 21 checkpoint. They are not rules gaps to fill casually while writing cards. Each needs a deliberate decision, then an update to [the production rulebook](unhinged-rules.md), card templates, and any affected card text.

| Topic | Current direction | Needed decision |
| --- | --- | --- |
| Unit or Character | **Character** is strongly favored. Current rules use Unit. | Lock the public card type and perform a reviewed terminology pass. |
| Owner or Controller | Prefer Owner and ordinary English. Existing legacy rules retain Controller. | Decide whether any production mechanic requires Controller. If not, remove it from the glossary. |
