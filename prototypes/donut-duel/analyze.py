"""Regenerate the deck sheet, opening-hand study, and report from saved runs."""
import json
import statistics
...
for p in range(2):
    ...
    openings.append(dict(leader=DECKS[p]['leader'], samples=10000, seed_start=1000000,
                         round1_one_cost_character=one, round1_character_cost_at_most_two=two))
(CASE / 'opening-hands.json').write_text(json.dumps(openings, indent=2) + '\n')

lines = ['# First duel test report', '', 'September 24, 2026 · Donut 0.1 / card pool revision 3', '',
         ...
         '| --- | ---: | ---: | ---: | ---: |']
batches = [('results.json', 'Original leaders / cautious pilot'),
           ('regular-only.json', 'Charge and ultimates disabled / cautious pilot'),
           ('aggressive.json', 'Original leaders / higher Health priority'),
           ('hoa-charge-experiment.json', 'Broader HOA Charge / higher Health priority')]
data = {}
for filename, label in batches:
    games = json.loads((CASE / filename).read_text())['games']
    data[filename] = games
    done = [g['rounds'] for g in games if g['winner'] is not None]
    lines.append(f'| {label} | {sum(g["winner"] == 0 for g in games)} | {sum(g["winner"] == 1 for g in games)} | {sum(g["winner"] is None for g in games)} | {statistics.mean(done):.1f} |')
lines += ['', 'The higher-Health-priority pilot increases the Health score weight by 50%; all card rules remain the same. The broader HOA Charge experiment changes only her Charge condition to: “The first time each Round one of your Characters survives damage during an opponent’s Turn, gain 1 Charge.” This alternative is an experiment, **not an adopted change** to the deck or Leader draft.', '',
          '## Leader use', '',
          '| Experiment | Florida regular activations | HOA regular activations | Florida ultimates | HOA ultimates | HOA Charge gained |',
          '| --- | ---: | ---: | ---: | ---: | ---: |']
for filename, label in batches:
    games = data[filename]
    totals = [sum(g['metrics'].get(k, 0) for g in games) for k in ('ability_0', 'ability_1', 'ultimate_0', 'ultimate_1', 'charge_1')]
    lines.append('| ' + label + ' | ' + ' | '.join(map(str, totals)) + ' |')
lines += ['', 'Counts above are totals across all eight runs, including unfinished runs. HOA never used an ultimate in any of the 24 runs with ultimates enabled. That makes the intended ultimate comparison inconclusive. Broadening her condition to surviving any opposing-turn damage increased Charge slightly but did not unlock an ultimate in this sample or change any result.', '',
          '## Opening consistency', '',
...
lines += ['', 'The HOA list intentionally has no 1-cost Characters; it usually starts establishing Characters in Round 2. That is a concrete deck difference to watch, not proof that it needs a 1-cost Character. It can still Play 1-cost Items on Round 1.', '',
